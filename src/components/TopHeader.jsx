...
const TopHeader = ({
  ...
  const [topHeaderNotificationCount, setTopHeaderNotificationCount] =
    useState(null);
  const [topHeaderNotificationCountClass, setTopHeaderNotificationCountClass] =
    useState(null);
  ...
  const [
    topHeaderNotificationMarkAllRead,
    setTopHeaderNotificationMarkAllRead,
  ] = useState(null);
  ...
  const fetchChildList = async () => {
    let response = await axios.get(
      `${BASE_URL}/enrollment/parent/getChildrenByParentId`,
      {
        headers: {
  ...
  const selectFranchisee = (e) => {
    ...
    if (e === 'All') {
      ...
      setFranchiseeId({
        ...franchiseeList?.filter((d) => parseInt(d.id) === parseInt(e))[0],
      });
  ...
  const selectChild = (e) => {
    ...
    // } else {
    setChildId({
      ...childList?.filter((d) => parseInt(d.id) === parseInt(e))[0],
    });
  ...
  const popover = (
    ...
    >
      ...
      <Popover.Body>
        {topHeaderNotification && topHeaderNotification.length !== 0 ? (
          topHeaderNotification.map((details, index) => (
            <div
              className={
                topHeaderNotificationCount ? 'notifitem unread' : 'notifitem'
              }
            >
              <div className="notifimg">
                <div className="notilink">
                  <div className="notifpic">
                    <img
                      src="/img/notification-ico1.png"
                      alt=""
                      className="logo-circle rounded-circle"
                    />
                  </div>
          ...
          <div className="text-center mb-5 mt-5">
            <strong>No Unread Notification Available</strong>
          </div>
        )}
      </Popover.Body>
      {topHeaderNotificationCount ? (
        <>
          <div className="totalmsg">
            You have{' '}
            {topHeaderNotificationCount ? topHeaderNotificationCount : 0} unread
            notifications
          </div>
          <div className="totalreadmsg" onClick={() => handleMarkRearAll()}>
            Mark All as Read
          </div>
        </>
      ) : (
  ...
  useEffect(() => {
    ...
    if (selectedFranchiseId !== 'undefined' && selectedFranchiseId !== null) {
      if (localStorage.getItem('selectedFranchise') === 'all') {
        ...
        setSelectedFranchisee(selectedFID);
        setFranchiseeId(
          franchiseeList.filter(
            (d) => parseInt(d.id) === parseInt(selectedFID)
          )[0]
        );
      }
    } else if (
              ...
              <Dropdown onSelect={selectChild}>
                <Dropdown.Toggle id="dropdown-basic">
                  {childList?.filter(
                    (d) =>
                      parseInt(d.id) ===
                      parseInt(localStorage.getItem('selectedChild'))
                  )[0]?.name || 'No Child Available'}
                </Dropdown.Toggle>
                <Dropdown.Menu>
                    ...
                    <div className="searchlisting cus-scr">
                      ...
                      <ul>
                        ...
                        {searchTraining?.map((trainingData) => (
                          <li>
                            <a
                              href={`/training-detail/${trainingData?.trainingId}`}
                              className="d-flex"
                            >
                        ...
                        {searchOperatingMannual?.map((operatingData) => (
                          <li>
                            <a
                              href={`/operatingmanual/?selected=${operatingData.id}`}
                              className="d-flex"
                            >
                        ...
                        {searchFileRepository?.map((fileRepoData) => (
                          <li>
                            <a
                              href={`/file-repository-List/${fileRepoData?.repository?.repository_files[0]?.categoryId}`}
                              className="d-flex"
                            >
                              {/* <img alt="" src={operatingData?.cover_image?operatingData.cover_image:'/img/notification-ico1.png'} className="logo-circle rounded-circle" /> */}
                              <span className="sec-cont">
                                <strong className="text-capitalize">
                                  {fileRepoData?.repository?.title}
                                </strong>
                              </span>
                            </a>
                          </li>
              ...
              <li className="user-col">
                <Dropdown>
                  ...
                  <Dropdown.Menu style={{ zIndex: '2000' }}>
                    ...
                    <Dropdown.Item
                      href={`/edit-user/${localStorage.getItem('user_id')}`}
                    >
                      My Profile
                    </Dropdown.Item>
