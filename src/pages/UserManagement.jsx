...
const UserManagement = () => {
          ...
          <>
            <div className="user-list">
              <span className="user-pic">
                <img
                  src={cell[0] === 'null' ? '../img/upload.jpg' : cell[0]}
                  alt=""
                />
              </span>
              <span className="user-name">
                <Link to={`/profile/${cell[4]}`}>{cell[1]}</Link>
                <small>{cell[2]}</small>{' '}
          ...
          <>
            ...
            cell[1] == 0 ? (
              <button
                ...
                onClick={() => navigate(`/child-enrollment-init/${cell[3]}`)}
              >
                New Children
              </button>
              ...
              <button
                ...
                onClick={() =>
                  navigate(`/children/${cell[3]}`, {
                    state: { franchisee_id: cell[2] },
                  })
                }
              >
                View Children
  ...
  const csvFileToArray = (csvTextData) => {
    const csvHeader = csvTextData
      .slice(0, csvTextData.indexOf('\n'))
      .split(',');
    const csvRows = csvTextData
      .slice(csvTextData.indexOf('\n') + 1)
      .split('\n');
  ...
  useEffect(() => {
    if (deleteResponse !== null) fetchUserDetails();
  }, [deleteResponse]);

  useEffect(() => {
    if (filter === '') fetchUserDetails();
  }, [filter]);
  ...
  return (
    <>
                          ...
                          <div className="extra-btn">
                            ...
                            <Dropdown className="filtercol me-3">
                              <Dropdown.Toggle
                                ...
                                variant="btn-outline"
                                onClickCapture={() => {
                                  if (openFilter === false) setOpenFilter(true);
                                }}
                              >
      ...
      {csvUploadFlag && (
        <Modal show={csvUploadFlag} onHide={() => setCsvUploadFlag(false)}>
          <Modal.Header>
          ...
          <Modal.Body>
            <div>
              <p>
                Choose a CSV file you want to extract and import the user data
                from.
              </p>
              <form>
                <input
          ...
          <Modal.Footer>
            <button className="modal-button" onClick={handleCSVImport}>
              Import CSV
            </button>
          </Modal.Footer>
        </Modal>
