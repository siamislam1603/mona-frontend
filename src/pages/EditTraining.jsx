...
const validateTrainingSettings = (trainingSettings) => {
  let errors = {};
  let { start_date, start_time, end_date, end_time } = trainingSettings;
...
const EditTraining = () => {
  ...
  const fetchFranchiseeUsers = async (franchisee_id, data) => {
    ...
    const response = await axios.post(
      `${BASE_URL}/auth/users/franchisees?franchiseeId=${franchisee_id}`,
      data,
      {
        headers: {
  ...
  const copyDataToStates = (training) => {
    ...
    setTrainingSettings((prevState) => ({
      ...
      end_date: training?.end_date
        ? moment(training?.end_date).format('YYYY-MM-DD')
        : '',
      ...
      assigned_users: training?.shares[0]?.assigned_users.includes('undefined')
        ? []
        : training?.shares[0]?.assigned_users,
    }));
    ...
    setFetchedVideoTutorialFiles(
      training?.training_files?.filter((file) => file.fileType === '.mp4')
    );
  ...
  const updateTraining = async (data) => {
    ...
    const response = await axios.put(
      `${BASE_URL}/training/updateTraining/${trainingId}`,
      data,
      {
        headers: {
          Authorization: 'Bearer ' + token,
    ...
    if (response.status === 201 && response.data.status === 'success') {
      ...
      if (typeof croppedImage === 'object') {
        ...
        imgSaveResponse = await fetch(croppedImage.getAttribute('src')).then(
          (res) => res.blob()
        );
        ...
        imgSaveResponse = await axios.patch(
          `${BASE_URL}/training/updateCoverImgString`,
          { croppedImage, trainingId },
          {
            headers: {
              Authorization: 'Bearer ' + token,
            },
  ...
  const handleDataSubmit = (event) => {
    ...
    let errorObj = EditTrainingFormValidation(
      trainingData,
      relatedFiles,
      fetchedRelatedFiles,
  ...
  const handleTrainingFileDelete = async (fileId, fileType) => {
    ...
    await axios
      .delete(`${BASE_URL}/training/deleteFile/${fileId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      .then(function () {
        if (fileType === 'related_files') {
          let newData = fetchedRelatedFiles.filter(
            (d) => parseInt(d.id) !== parseInt(fileId)
          );
          setFetchedRelatedFiles(newData);
        } else if (fileType === 'video_files') {
          let newData = fetchedVideoTutorialFiles.filter(
            (d) => parseInt(d.id) !== parseInt(fileId)
          );
  ...
  useEffect(() => {
    setDocError(docFileError?.map((errObj) => errObj?.error[0]?.message));
  }, [docFileError]);
  ...
  return (
    <div style={{ position: 'relative', overflow: 'hidden' }}>
                      ...
                      <Row>
                        ...
                        <Col md={6} className="mb-3">
                          <Form.Group>
                            ...
                            <Select
                              ...
                              value={
                                trainingCategory.filter(
                                  (c) => c.id === trainingData.category_id
                                ) ||
                                trainingCategory.filter(
                                  (c) => c.id === trainingData.category_id
                                )
                        ...
                        <Col md={6} className="mb-3">
                          <Form.Group className="relative">
                            ...
                            {errors && errors.time_required_to_complete && (
                              <span className="error">
                        ...
                        <Col md={6} className="mb-3">
                          <Form.Group>
                            ...
                            <Select
                              ...
                              value={trainingFormData?.filter(
                                (d) =>
                                  parseInt(d.id) ===
                                  trainingData?.training_form_id
                              )}
                              onChange={(event) => {
                                setTrainingData((prevState) => ({
                                  ...prevState,
                        ...
                        <Col md={6} className="mb-3">
                          <Form.Group>
                            ...
                            {typeof croppedImage === 'string' ? (
                              <>
                                <img
                                  className="cover-image-style"
                                  src={croppedImage}
                                  alt="training cover image"
                                />
                              </>
                            ) : (
                              <></>
                              ...
                              {fetchedRelatedFiles &&
                                fetchedRelatedFiles.map((file, index) => {
                                  return (
                                    <div className="file-container">
                                      <img
                                        className="file-thumbnail-vector"
                                        style={{ marginRight: '10px' }}
                                        src={`../img/book-ico.png`}
                                        alt={`${file.videoId}`}
                                      />
                                      <a href={file.file}>
                                        <p className="file-text">{`${fetchRealatedFileName(
                                          file.file
                                        )}`}</p>
                                      </a>
                                      <img
                ...
                <Row>
                  <Col lg={3} sm={6}>
                    <Form.Group>
                      ...
                      <Form.Control
                        ...
                        placeholder={
                          trainingSettings?.start_date
                            ? moment(trainingSettings?.start_date).format(
                                'DD/MM/YYYY'
                              )
                            : 'dd/mm/yyyy'
                        }
                  ...
                  <Col lg={3} sm={6} className="mt-3 mt-lg-0">
                    <Form.Group>
                      ...
                      <Form.Control
                        ...
                        placeholder={
                          trainingSettings?.end_date
                            ? moment(trainingSettings?.end_date).format(
                                'DD/MM/YYYY'
                              )
                            : 'dd/mm/yyyy'
                        }
                      ...
                      />
                      {trainingSettingErrors?.end_date !== null && (
                        <span className="error">
                          {trainingSettingErrors?.end_date}
                        </span>
                      )}
                    </Form.Group>
                  </Col>
                  <Col lg={3} sm={6} className="mt-3 mt-lg-0">
                    <Form.Group>
                      ...
                      />
                      {trainingSettingErrors?.end_time !== null && (
                        <span className="error">
                          {trainingSettingErrors?.end_time}
                        </span>
                      )}
                    </Form.Group>
                  </Col>
                        ...
                        <div className="new-form-radio-box">
                          <label for="all">
                            <input
                              type="radio"
                              checked={
                                trainingSettings?.send_to_all_franchisee ===
                                true
                              }
      ...
      {createTrainingModal && (
        <Modal show={createTrainingModal}>
          <Modal.Header>
            <Modal.Title>Updating Training</Modal.Title>
          </Modal.Header>
