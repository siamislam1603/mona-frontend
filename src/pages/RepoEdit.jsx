...
const RepoEdit = () => {
  ...
  const GetData = async () => {
    let response = await axios.get(
      `${BASE_URL}/fileRepo/fileInfo/${Params.id}`,
      {
        headers: {
    ...
    if (response.status === 200 && response.data.status === 'success') {
      if (typeof data.image === 'string') {
        response = await axios.patch(
          `${BASE_URL}/fileRepo/updateFilePath/${Params.id}`,
          { filesPath: data.image },
          {
            headers: {
              Authorization: 'Bearer ' + token,
            },
        ...
        response = await axios.post(
          `${BASE_URL}/fileRepo/data/saveImageData`,
          dataObj,
          {
            headers: {
              Authorization: 'Bearer ' + token,
  ...
  const childList = async () => {
    const token = localStorage.getItem('token');
    let response = await axios.get(
      `${BASE_URL}/enrollment/listOfChildren?childId=${JSON.stringify(
  ...
  const getFileCategory = async () => {
    const token = localStorage.getItem('token');
    const response = await axios.get(`${BASE_URL}/fileRepo/files-category/`, {
      headers: {
        Authorization: 'Bearer ' + token,
      },
    });
