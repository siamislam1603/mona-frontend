...
const AvailableTraining = ({ filter, selectedFranchisee, setTabName }) => {
  ...
  const fetchAvailableTrainings = async () => {
    try {
      ...
      const response = await axios.get(
        `${BASE_URL}/training/assigeedTraining/${user_id}?category_id=${filter.category_id}&search=${filter.search}&limit=${page}&filter=${filter.filter}`,
        {
          headers: {
      ...
      if (response.status === 200 && response.data.status === 'success') {
        ...
        let withDueDate = searchedData.filter(
          (d) => d.training.end_date !== null
        );
        let withoutDueDate = searchedData.filter(
          (d) => d.training.end_date === null
        );
      ...
      : setFormSettings((prevState) => ({
          ...
          assigned_roles:
            localStorage.getItem('user_role') === 'franchisee_admin'
              ? training?.shares[0]?.assigned_roles.filter(
                  (d) => d !== 'franchisee_admin'
                )
              : training?.shares[0]?.assigned_roles.filter(
                  (d) => d !== 'coordinator' && d !== 'franchisee_admin'
                ),
          assigned_franchisee: [selectedFranchisee],
          applicable_to: training?.shares[0]?.applicable_to,
          send_to_all_franchisee: false,
  ...
  trainingWithDueDate &&
    console.log('TRAINING WITH DUE DATE:', trainingWithDueDate);
  trainingWithoutDueDate &&
    console.log('TRAINING WITHOUT DUE DATE:', trainingWithoutDueDate);
  return (
    <>
      {topErrorMessage && (
        <p
        ...
        <div className="training-column">
          ...
          <Row>
            {trainingWithDueDate?.length > 0 && (
              <div>
                <h3 className="title-sm mb-3 mt-3">
                  <strong>Training with due date</strong>
                </h3>
                    ...
                    return (
                      <Col lg={4} md={6}>
                        <div className="item mt-3 mb-3">
                          <div className="pic">
                            <a href={`/training-detail/${item.training.id}`}>
                              <img src={item.training.coverImage} alt="" />{' '}
                          ...
                          <div className="fixcol">
                            ...
                            <div className="iconame">
                              <a href={`/training-detail/${item.training.id}`}>
                                {item.training.title.length > 40
                                  ? item.training.title.slice(0, 40) + '...'
                                  : item.training.title}
                              </a>
                              <div className="datecol">
                                {item.training.end_date !== null && (
                                  <span className="red-date">
                                    Due Date:{' '}
                                    {moment(item.training.end_date).format(
                                      'DD/MM/YYYY'
                                    )}
                                  </span>
                                )}
                            ...
                            <div className="cta-col">
                              {localStorage.getItem('user_role') !==
                                'educator' &&
                                isTrainingExpired(item.training.end_date) ===
                                  false && (
                                  <Dropdown>
                                    <Dropdown.Toggle
                                      variant="transparent"
            ...
            {trainingWithoutDueDate?.length > 0 && (
              <h3 className="title-sm mb-3 mt-3">
                <strong>Training without due date</strong>
              </h3>
            )}
            {trainingWithoutDueDate
            ...
            dueDataTraining === false &&
            nodueData === false &&
            fullLoaderStatus === false ? (
              <div className="text-center mb-5 mt-5">
                {' '}
                <strong>No training assigned to you.</strong>{' '}
              </div>
            ) : null}
            {/* {availableTrainingData
                            ...
                            'franchisee_admin' ? (
                              ...
                              >
                                <Form.Check
                                  ...
                                  label="All Roles"
                                  checked={
                                    formSettings.assigned_roles.length === 1
                                  }
                                  onChange={() => {
                                    if (
                                      formSettings.assigned_roles.includes(
                        ...
                        <Form.Group>
                          ...
                          <div className="select-with-plus">
                            <Multiselect
                              ...
                              className="multiselect-box default-arrow-select"
                              selectedValues={
                                fetchedFranchiseeUsers?.filter((d) =>
                                  formSettings?.assigned_users.includes(
                                    d.id + ''
                                  )
                                ) || ''
