...
function getFilteredChildren(children) {
  ...
  if (
    ...
    if (localStorage.getItem('user_role') === 'educator') {
      ...
      let filteredChildrenByEmail = children.map((child) =>
        child.users.map((c) => c.email === loggedInUserEmail)
      );
      filteredChildrenByEmail = filteredChildrenByEmail.map((t) =>
        t.includes(true)
      );
      newFilteredList = children.filter(
        (child, index) => filteredChildrenByEmail[index] === true
      );
    } else {
      let franchisee_id = localStorage.getItem('franchisee_id');
      newFilteredList = children.filter(
        (children) => children.franchisee_id === parseInt(franchisee_id)
      );
    }
  }
...
const Children = () => {
  ...
  const init = async () => {
    ...
    const franchiseeId =
      location?.state?.franchisee_id || localStorage.getItem('franchisee_id');
    setFranchiseId(franchiseeId);

    // FETCHING PARENT DATA
    const fetchParentData = async () => {
      const response = await axios.get(
        `${BASE_URL}/auth/user/${params.parentId}`
      );
    ...
    let response = await axios.get(
      `${BASE_URL}/enrollment/children/${params.id}`,
      {
        headers: {
  ...
  const sendInitiationMail = async (childId) => {
    ...
    const response = await axios.post(
      `${BASE_URL}/enrollment/send-mail/${params.id}`,
  ...
  const DeactivateChild = async (id) => {
    const response = await axios.patch(
      `${BASE_URL}/enrollment/deactivate-reactivate-child/${id}`,
      {},
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const fetchParents = async (id) => {
    ...
    let response = await axios.get(
      `${BASE_URL}/enrollment/child/parent/${childId || id}`,
      {
        headers: {
          authorization: `Bearer ${localStorage.getItem('token')}`,
        },
    ...
    if (response.status === 200) {
      ...
      response = await axios.get(
        `${BASE_URL}/enrollment/child/franchise/${childId || id}`,
        {
          headers: {
            authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const getEducatorsByFranchisee = async (id) => {
    //   Educators list
    let eduResponse = await axios.get(
      `${BASE_URL}/role/franchisee/coordinator/franchiseeID/${id}/educator`,
      {
        headers: {
          authorization: `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const getParentsByFranchisee = async (id) => {
    //   Parents list
    let CpResponse = await axios.get(
      `${BASE_URL}/role/franchisee/coordinator/franchiseeID/${id}/guardian`,
      {
        headers: {
          authorization: `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const fetchAndSaveEducatorData = async (passedChildId, passedParentId) => {
    let response = await axios.get(
      `${BASE_URL}/enrollment/child/${passedChildId}?parentId=${passedParentId}`,
      {
        headers: {
          Authorization: 'Bearer ' + localStorage.getItem('token'),
        },
  ...
  const rowEvents = {
    onClick: (e, row, rowIndex) => {
      ...
      if (e.target.text === 'Deactivate' || e.target.text === 'Activate') {
        if (
          window.confirm(
            `Are you sure you want to ${e.target.text} this child?`
          )
        ) {
  ...
  const products = educators.map((educator) => ({
    ...
    name:
      educator.fullname +
      ',' +
      (educator.profile_photo ? educator.profile_photo : '../img/user.png'),
  ...
  const productsTow = childrenList?.map((child, index) => ({
    ...
    EnrollFlag: {
      enrollFlag: child.isChildEnrolled,
      childId: child.id,
      initiationFlag: child.isEnrollmentInitiated,
    },
            ...
            {cell.length === 0 ? (
              <div className="user-list">
                <Button
                  ...
                  onClick={() => handleShow(cell[20], cell.educators)}
                  style={{ backgroundColor: '#3e5d58', color: 'white' }}
                >
                  Add Educator
                </Button>
              </div>
              ...
              <div
                ...
                style={{ maxHeight: '105px', overflowY: 'scroll' }}
              >
                {cell[0].trim() != 'undefined' && (
                  <div className="user-list">
                    <span className="user-pic">
                      ...
                      <img
                        src={
                          cell[10] === 'undefined' || cell[10].trim() === 'null'
                            ? '../img/upload.jpg'
                            : cell[10]
                        }
                      />
                ...
                {cell[1].trim() != 'undefined' && (
                  <div className="user-list">
                    <span className="user-pic">
                      <img
                        src={
                          cell[11] === 'undefined' || cell[11].trim() === 'null'
                            ? '../img/upload.jpg'
                            : cell[11]
                        }
                      />
    ...
    },
    {
      ...
      formatter: (cell) => {
        ...
        return (
          <>
            {cell.length === 0 ? (
              <div className="user-list">"No Co-Parents Assigned Yet!"</div>
            ) : (
        ...
        return (
          <>
            <div className="user-list">
              ...
              <span className="user-name">{cell[0]}</span>
            </div>
          </>
        );
      },
                    ...
                    {childrenList.length === 0 ? (
                      <div className="center-division">
                        {localStorage.getItem('user_role') !== 'guardian' ? (
                          <>
                            <p className="center">
                              No child connected to this parent.
                            </p>
                            <p className="center">
                              Click on <strong>Add Child</strong> to add one.
                            </p>
                          </>
                        ) : (
                          <>
